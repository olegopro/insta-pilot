...
widgets/
  ...
  automation-launch-cockpit/  кокпит запуска: действие + цели + интерактивный таймлайн + Старт
...
- `time-distribution-editor` — интерактивный таймлайн запуска: по кубику на цель, drag
  (`v-touch-pan`), окно с зажимом minWindow; чистые функции в `lib/timeDistribution.ts` (snap/MIN_GAP=60с).
...
**Пост-релизный UX/корректность-инкремент (2026-06-22, проверен живьём + юнит-тестами):** ручной
интерактивный таймлайн запуска (`time-distribution-editor`: drag-кубики по цели, окно с зажимом
minWindow) — контракт `/start` с пер-целевыми `offset_seconds` → `ScheduleActionItemsJob` →
`ActionSchedulerService::buildItemsWithOffsets` (ключ `parsed_target_id`, MIN_GAP=60с, без jitter,
покрыт unit-тестами); skip уже-лайкнутых/обработанных при парсинге (`ParseTargetsJob`, `has_liked` +
добор страниц до `target_limit`); счётчик `collected_targets_count` для черновика. Дедуп планировщика
усилён (`insertOrIgnore` + пред-фильтр `media_pk`), терминальное событие обнуляет `current_action`.
