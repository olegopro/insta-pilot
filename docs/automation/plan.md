...
Архитектура и канонические решения — [architecture.md](architecture.md).
...
## Phase 0 — Инфраструктурные предусловия (БЛОКЕРЫ) ✅ ВЫПОЛНЕНО
...
## Phase 1 — MVP-вертикаль: комментарии по хэштегу/гео, полу-ручной режим ✅ ВЫПОЛНЕНО

> Реализовано и проверено (BE 231 / PY 176 тестов, vue-tsc чисто, UI вживую в Chrome, API-roundtrip).
> Живой запуск против реального IG-аккаунта не выполнялся — нужны заданные дневные лимиты на аккаунт.
...
- **Предусловие:** подтвердить формат `feed/user/{pk}/` на живом аккаунте (чек-лист
  `../debug-protocol.md`) — от него зависит экстрактор.
...
## Phase 2 — Полностью автоматический режим ✅ ВЫПОЛНЕНО

> Реализовано и проверено живьём: `full_auto` парсит и САМ планирует исполнение (без ручного `/start`),
> UI уходит на «Задачи» с realtime-прогрессом; выбор действия (комментарий/лайк); авто-refresh review.
...
## Phase 4 — Рабочие часы и лимиты как полноценный UX ✅ ВЫПОЛНЕНО

> Проверено живьём: вкладка «Настройки» грузит лимиты (GET) и сохраняет (PUT, comment 30→25 в БД);
> модалка рабочих часов (сетка 7×24 + TZ) сохраняет (PUT, окно 9–21 в БД) и **enforced** —
> планировщик сдвинул `run_at` закрытого часа 08:42 на `09:00` (начало окна). Багов не найдено.
...
## Phase 5 — Масштаб и надёжность ✅ ВЫПОЛНЕНО (база)

> `automation-worker` масштабируется (`--scale automation-worker=N`, проверено: 2 контейнера Up),
> многоворкерность безопасна (FOR UPDATE SKIP LOCKED + CAS-claim), timeout 180 (> job 120 < retry 300).
> Реальный параллелизм — по разным аккаунтам (с одним аккаунтом `account_lock` сериализует).
...
## Phase 6 — Подписки/отписки ✅ ВЫПОЛНЕНО

> follow (по хэштег-парсингу) и unfollow (источник `my_following` = текущие подписки аккаунта) —
> проверены живьём (реальные подписка + отписка). comment/like/follow/unfollow — один движок, плагины.
...
разложить на параллельных субагентов в изолированных worktree (см. `../orchestration/playbook.md`):
