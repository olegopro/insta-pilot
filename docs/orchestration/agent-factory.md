...
5. **Параллельность ограничена ДВУМЯ воротами: потолком обёртки и аккаунтами.** Первое (и до
   2026-06-24 — главное) — `KIRO_MAX_CONCURRENCY` обёртки `kiro-cli`: дефолт **3** реально
   одновременных вызовов на MCP-сервер (раньше синхронная обёртка держала фактически 1 — отсюда
   прежняя «полная сериализация», НЕ из-за аккаунта). Второе — backend per-account spacing (§5).
   Эффективное окно = `min(KIRO_MAX_CONCURRENCY, что_терпит_бэкенд)`. На РАЗНЫХ аккаунтах/worktree
   параллель настоящая (до `min(3, k)`); на ОДНОМ аккаунте 3 одновременных — ПОДТВЕРЖДЕНЫ живым тестом
   2026-06-24 (3 модели разом, все exit=0, без throttle; §5.1). При `A_concurrent` выше окна лишние ждут
   слот лимитера/spacing-очередь (M/D/1, ρ>1), wall-clock не падает, токены за висящий контекст тратятся.
...
- **При текущем эффективном `k=1`** (активен один Kiro-аккаунт, остальные `disabled`) 50–80
  структур с self-healing — это РАБОЧИЙ ДЕНЬ (2.5–6.7 ч), а не минуты. С async-обёрткой (2026-06-24)
  один аккаунт может тянуть до 3 одновременных, ЕСЛИ бэкенд это терпит (не перемеряно — §5.1); иначе
  near-linear рычаг прежний: поднимать `k` (re-enable аккаунтов), а не число агентов.
...
**Сначала — потолок обёртки (с 2026-06-24).** Обёртка `kiro-cli` стала async и кэпит конкурентность на
`KIRO_MAX_CONCURRENCY` (дефолт **3**) на MCP-сервер: 4-й+ одновременный вызов ждёт слот. Это ПЕРВЫЙ
лимит — раньше синхронная обёртка держала фактически 1 (блокирующий `subprocess` держал event loop),
отсюда прежняя «полная сериализация», НЕ из-за аккаунта. Backend-слой ниже — ВТОРИЧНЫЙ: бьёт, только
если бэкенд начинает троттлить раньше, чем упрёшься в потолок обёртки. **Живой тест 2026-06-24:** один
аккаунт ВЫДЕРЖАЛ 3 реально одновременных тяжёлых вызова (opus max + sonnet max + glm, все `exit=0`, без
throttle) — т.е. прежняя «per-account spacing ~8.5с» оказалась артефактом СИНХРОННОЙ обёртки, а не
лимитом бэкенда (для k=1 при concurrency=3 не подтвердилась).

Backend per-account spacing на бо́льшей конкурентности (>3) или на лёгких частых вызовах (CountTokens) —
всё ещё возможен (не измеряно); throttle-guard ловит реальный 429 → cooldown.
...
- **Точка убыточности параллелизма: `A_concurrent > k`.** Аккаунт при scope=`auth` — M/D/1-сервер.
  ...
  law](https://en.wikipedia.org/wiki/Little's_law)). **Окно параллельности = `min(KIRO_MAX_CONCURRENCY,
  что_терпит_бэкенд)`** (раньше синхронная обёртка занижала фактически до 1). На разных аккаунтах/worktree
  — до `min(3, k)`; на одном аккаунте concurrency=3 ПОДТВЕРЖДЕНА живьём (2026-06-24, все exit=0).

### 5.2 Формула обещания скорости (вписать в `playbook.md`)

```
wall_clock_волны ≈ (N_структур × вызовов_на_структуру / окно) × call_duration_сек

окно = min(KIRO_MAX_CONCURRENCY, что_терпит_бэкенд)   # дефолт обёртки 3; разные аккаунты/worktree → min(3,k)
...
```
(Таблица ниже считалась для `окно=k`; теперь `k`→`окно=min(3, …)`. Single-account конкурентность на
бэкенде после async-фикса НЕ перемеряна — числа для `k=1` могут улучшиться, если аккаунт терпит >1.)
...
**Рычаги near-linear speedup:** потолок обёртки `KIRO_MAX_CONCURRENCY` (дефолт 3, в нашем контроле) +
число живых аккаунтов `k` (доп. профили `kiro-cli login`). Окно = `min(KIRO_MAX_CONCURRENCY, что терпит
бэкенд)`; агенты сверх окна бесполезны. Озвучивать пользователю реалистичную вилку, не «быстро».
...
маршрутизатора с round-robin (как в мёртвом прокси) больше нет. Живой тест 2026-06-24 показал: один
аккаунт держит 3 одновременных (прежняя оценка ~8.5с spacing была артефактом синхронной обёртки, §5.1);
рычаги near-linear ускорения — потолок обёртки `KIRO_MAX_CONCURRENCY` (дефолт 3) и число живых
аккаунтов `k` (§5.2).
...
- Реальная одновременность = `min(KIRO_MAX_CONCURRENCY, k×терпимость_бэкенда)` (обёртка async с
  2026-06-24, дефолт 3; раньше синхронная держала 1). Один аккаунт держит 3 одновременных (ПОДТВЕРЖДЕНО
  2026-06-24) → при `k=1` окно=3, а не 1: 50–80 структур ≈ 0.8–2.2 ч (§5.2, ~втрое быстрее прежней
  оценки 2.5–6.7 ч, считавшейся для окна=1). Сверх окна число агентов не помогает.
...
| Окно параллельности = `min(KIRO_MAX_CONCURRENCY, k)` (обёртка async, дефолт 3) | Сверх окна агенты висят в лимитере/spacing-очереди (M/D/1, A>окна), скорость не растёт, токены тратятся |
