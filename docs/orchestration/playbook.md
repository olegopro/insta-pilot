...
### Оговорки
- **Per-account spacing Kiro**: Kiro-бэкенд держит паузу per-account (per-auth). На ОДНОМ Kiro-аккаунте
  параллельные вызовы сериализуются — реального параллелизма по одному аккаунту нет; тяжёлый
  сустейнед-объём на одном аккаунте рискован. Для конкуренции разводить нагрузку по каналам
  (Kiro/Codex/Claude-Agent), а не по моделям одного Kiro.
