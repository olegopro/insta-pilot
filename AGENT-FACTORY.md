...
|---|---|---|
| **MVP-0 (уже есть)** | `dashboard-builder` + `parallel-feature-build` + `ORCHESTRATION.md` + делегаты `kiro-cli` (+ Codex) | 5 дашбордов параллельно из worked-example работают |
...
- `ORCHESTRATION.md` — детальный плейбук параллелизма (контекст, git, worked-example, делегаты `kiro-cli` (+ Codex))
