...
Это надстройка над уже существующими `ORCHESTRATION.md` (плейбук параллелизма),
...
PostgreSQL + Redis + Laravel Reverb, всё в Docker. Агенты-исполнители ходят через VibeProxy `:8318`
(Anthropic/OpenAI-совместимый) на модели Kiro.
...
- **При текущем эффективном `k=1`** (активен один Antigravity/Kiro-аккаунт, остальные `disabled`) 50–80
...
### 5.2 Формула обещания скорости (вписать в `ORCHESTRATION.md`)
...
**Единственный рычаг near-linear speedup — поднять `k`** (re-enable Antigravity/Kiro-аккаунтов в конфиге
прокси). Агенты сверх `k` бесполезны. Озвучивать пользователю реалистичную вилку, не «быстро».
...
### 5.5 Кэш, affinity и режимы конкуренции (проверено по исходникам CLI Proxy)

**Prompt caching — важная оговорка.** Авто-инъекция Anthropic `cache_control` (лимит 4 breakpoints)
реализована в НАТИВНОМ Claude-executor (`internal/runtime/executor/claude_executor.go`), у xAI — через
`prompt_cache_key`. Но в трансляторе Kiro (`internal/translator/kiro/`) обработки `cache_control` НЕТ.
Вывод: кэширование стабильного префикса (`CONTRACTS.md`) окупается на нативных Claude-аккаунтах, а на
пути **Kiro — скорее не работает**. Не закладывать cache-экономию как данность для Kiro; при остром
токен-бюджете рассмотреть нативные Claude-аккаунты под оркестратор/reviewer.

**Affinity / sticky.** Для cache-locality (где кэш есть) и стабильной привязки тайлов в дашборде (§8)
нужен sticky routing на тот же аккаунт — даёт `affinity.enabled: true` + session-affinity проекта
(`X-CLIProxy-Session-ID`/JSON body). Пробросить воркерам стабильный `session_id` через конвенцию.

**Режимы конкуренции (настройки CLI Proxy).**
- `routing.strategy`: `round-robin` (дефолт) / `fill-first` — выбор аккаунта при НЕСКОЛЬКИХ; при `k=1`
  неактуально.
- `antigravity-custom.request-spacing` (scope `auth`/`provider`, `min`/`max`) — единственный рычаг на
  одном аккаунте. Жёсткого cap «N одновременно» НЕТ: spacing включён (2–15с) ≈ последовательно;
  уменьшить/выключить → конкурентные вызовы на одном аккаунте (риск rate-limit/антибот Kiro). Это и
  есть «один / несколько одновременно / по очереди» — регулируется spacing, а не отдельной настройкой.
...
2. **Integrator-агент** (фолбэк): один финальный агент/оркестратор последовательно дописывает реестры
   по сводкам воркеров (как в `ORCHESTRATION.md` шаг 3).
...
> typecheck/lint гоняются в общем контейнере, Playwright — на финальной интеграции (см. `ORCHESTRATION.md`
...
(на VibeProxy: spacing 2–15с × итерации = простой + сжигание квоты; границы Dagger-цикла надо вводить
...
- **Прямой Pusher-HTTP-API Reverb из shell-хука** требует ручного HMAC — хрупко. Идти через Laravel-роут
  с `broadcast()`.
- **Стабильный session_id** переиспользовать из session-affinity проекта — иначе события расползутся по
  тайлам.
...
**Durable-движок:** апрув async, состояние волны в durable-store, НЕ в живом HTTP к `:8318` (иначе
протухнут Kiro-OAuth-токены и gateway-таймауты убьют hold). На resume валидировать hash плана — если
...
- **Разорвать lethal trifecta** (приватные данные + untrusted-вход + внешний канал): гонять воркер в Docker
  с network-egress allowlist (packagist/npm/github + только VibeProxy:8318 + git-remote, остальное deny) и
...
- **DLP-фильтр в VibeProxy:** вычищать паттерны APP_KEY/DB-кредов/токенов на исходящих запросах к Kiro и
  до записи в observability — прокси уже на пути запроса, естественная точка контроля.
...
|---|---|---|
| **MVP-0 (уже есть)** | `dashboard-builder` + `parallel-feature-build` + `ORCHESTRATION.md` + 3 Kiro-канала | 5 дашбордов параллельно из worked-example работают |
...
1. **Prompt caching на пути Kiro — частично закрыто исходниками (см. §5.5).** В трансляторе Kiro
   ...
   полезным для подтверждения; зафиксировать в `DEBUG_PROTOCOL.md`. Альтернатива при остром бюджете —
...
- `ORCHESTRATION.md` — детальный плейбук параллелизма (контекст, git, worked-example, Kiro-каналы)
