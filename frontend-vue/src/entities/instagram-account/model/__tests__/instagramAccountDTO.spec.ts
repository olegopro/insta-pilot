...
describe('instagramAccountDTO.toLocalList', () => {
  ...
  it('возвращает пустой массив при пустом входе', () => {
    expect(instagramAccountDTO.toLocalList([])).toEqual([])
  })
})

describe('instagramAccountDTO.toLocalDetailed', () => {
  it('включает базовые поля и дополнительные (userPk, followers, following)', () => {
...
describe('instagramAccountDTO.toLocalDeviceProfiles', () => {
  it('маппит массив профилей устройств', () => {
...
describe('instagramAccountDTO.toApiRequest', () => {
  it('преобразует camelCase в snake_case для отправки на API', () => {
