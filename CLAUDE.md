...
полу-ручной и полностью-авто, лимиты/часы/cooldown/идемпотентность. Остаётся (опц.): Phase 5 (масштаб —
несколько воркеров), Phase 6 (подписки/отписки — нужны новые Python-эндпоинты), настройка LLM-провайдера.
