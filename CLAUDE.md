...
> ⚠️🔴 **ВАЖНО (обновлено по факту сессии): связка «Kiro через VibeProxy» ЗАБАНИЛА аккаунт Kiro.**
> Это неофициальный мост — credentials Kiro руками вставлены в локальный прокси VibeProxy (`:8318`),
> поверх `delegate_edit`/`ask`; НЕ официальный CLI/MCP. Результат: `HTTP 403 … temporarily is suspended …
> security precaution` ПОСРЕДИ работы, канал до конца сессии недоступен. **VibeProxy-Kiro в оркестрацию
> НЕ брать** до появления ЛЕГИТИМНОЙ (официальной) реализации Kiro. Надёжный костяк: **Codex (официальный
> MCP) + Claude-Agent + главная модель** (за всю сессию 0 сбоев; Codex вынес Python+Laravel, Claude —
> фронт). Проблема НЕ в мультиагентности — падает один конкретный канал. Полный красный warning и хроника
> — вверху `ORCHESTRATION-RETROSPECTIVE.md`. Разделы ниже про приоритет моделей Kiro/маршрутизацию
> оставлены как историческая база — применять ТОЛЬКО если канал станет официальным.
