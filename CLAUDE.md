...
**Блок «Автоматизация» завершён.** Все 4 действия (comment/like/follow/unfollow) проверены на живом
аккаунте; полу-ручной + полностью-авто; лимиты, рабочие часы (UI+enforcement), настройки, очереди,
диспетчер, cooldown, идемпотентность, realtime, масштабирование. Остаётся только настроить
LLM-провайдера в «Настройки LLM» (нужен API-ключ владельца — интеграция готова, без ключа `resolve`
коммента даёт понятную ошибку; исполнение с готовым текстом работает).
