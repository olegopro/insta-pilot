...
**Follow-up:** review-таблица не авто-обновляется после async-парсинга (фронт зовёт `fetchTargets` до
завершения job — нужна подписка на прогресс/поллинг). Дальше — Phase 2+ плана (полностью-авто, UX
часов/лимитов, масштаб, подписки).
