...
3. **Gemini Flash** (`mcp__gemini-cli`) — опц. дешёвый/быстрый кодер (Kiro остаётся ПЕРВЫМ). Для НАПИСАНИЯ
   кода — `Gemini 3.5 Flash (High)`: обёртка АВТО-подмешивает `CODESTYLE.agent.md` слоя (выжимка из
   линтеров — отступы/кавычки/именование), т.к. слабой модели нужен явный стиль. Style-guide-файлы лежат
   в корнях слоёв: `frontend-vue/`, `backend-laravel/`, `python-service/` → `CODESTYLE.agent.md` (обновлять
   при смене линтер-конфигов). Триггер автоматический (в имени модели «flash»).

Ревью оркестратора (`/code-review`, `/simplify`) — в его сессии; «второе мнение» можно отдать Codex,
синтез/решение — за Opus.

Параллелизм: независимые куски — разом. **kiro-cli (с 2026-06-24): обёртка async, прежний синхронный
боттлнек снят** — допускает до `KIRO_MAX_CONCURRENCY` (дефолт 3) одновременных вызовов. На РАЗНЫХ
аккаунтах/worktree параллель реальная (до `min(3, k)`); на ОДНОМ аккаунте 3 одновременных —
**ПОДТВЕРЖДЕНО живым тестом 2026-06-24** (opus max + sonnet max + glm, старт в одну секунду, все exit=0,
без throttle; прежняя «сериализация» была из-за обёртки, не аккаунта; детали — `playbook.md` §6).
Параллель по умолчанию
ВЫКЛ, включается ЯВНОЙ командой и ТОЛЬКО через `delegate_batch` (один вызов → N задач конкурентно внутри
сервера; отдельные `delegate` одним сообщением Claude Code сериализует). Вторично — backend rate-limit
(throttle-guard); >3 разводить по РАЗНЫМ каналам/директориям.
