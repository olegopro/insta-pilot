...
движок (расширяемость плагинами подтверждена). **Unfollow**: плагин+эндпоинт есть, но в UI disabled —
хэштег-парсинг не годится для отписки (нужен источник «мой список подписок» аккаунта — отдельная
вертикаль). Лимиты follow/unfollow есть в форме настроек.
Остаётся (опц.): Phase 5 (масштаб — несколько воркеров для параллелизма по аккаунтам); источник «мои
подписки» для unfollow; настройка LLM-провайдера (для авто-генерации текста комментария).
